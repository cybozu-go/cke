...
func testKubernetes() {
	...
	"spec": { "nodeSelector": { "kubernetes.io/hostname": "%s" }}
		...
		_, stderr, err = kubectl("run",
			"-n="+namespace, "--image=ghcr.io/cybozu/ubuntu:24.04", "--overrides=\"$(cat "+overrideFile+")\"", "--restart=Never",
			"client", "--", "pause")
	...
	It("has node DNS resources", func() {
		...
		_, stderr, err = kubectl("run", "-n="+namespace, "--image=ghcr.io/cybozu/ubuntu:24.04", "--restart=Never",
			"client", "--", "pause")
		Expect(err).NotTo(HaveOccurred(), "stderr: %s", stderr)
		Eventually(func() error {
