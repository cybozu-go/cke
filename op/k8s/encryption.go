...
func getEncryptionConfiguration(ctx context.Context, inf cke.Infrastructure) (*apiserver.EncryptionConfiguration, error) {
	data, err := getEncryptionSecret(ctx, inf, "aescbc")
	if err != nil {
		return nil, err
	}

	aescfg := new(apiserver.AESConfiguration)
