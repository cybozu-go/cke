...
type FileOption func(*fileData)

func WithFileMode(mode int64) FileOption {
	return func(f *fileData) {
		f.mode = mode
	}
}
...
func (c *FilesBuilder) AddFile(ctx context.Context, name string, f func(context.Context, *cke.Node) ([]byte, error), opts ...FileOption) error {
	var mu sync.Mutex
	...
	fd := fileData{name: name, mode: defaultFileMode, dataMap: dataMap}
...
func (c *FilesBuilder) AddKeyPair(ctx context.Context, name string,
	f func(context.Context, *cke.Node) (cert, key []byte, err error),
) error {
	var mu sync.Mutex
	...
	c.files = append(c.files, fileData{name: name + ".crt", mode: defaultFileMode, dataMap: certMap})
	c.files = append(c.files, fileData{name: name + ".key", mode: defaultFileMode, dataMap: keyMap})
	return nil
}
