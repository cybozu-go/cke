...
2. executes the steps sequentially:
    1. if Pod eviction is required in the step and the machine is used as a Node of the Kubernetes cluster:
        ...
        2. checks the existence of Job-managed Pods on the Node, deleting those permitted by `deletable_job_pod_selector`. If any Job-managed Pod not permitted for deletion remains, uncordons the Node immediately and processes it again later.
...
If a running Job-managed Pod not permitted for deletion remains, CKE interrupts the eviction, uncordons the target machine, and retries later.
If `deletable_job_pod_selector` is not given, Job-managed Pods are never deleted.
