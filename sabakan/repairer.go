...
func Repairer(machines []Machine, repairEntries []*cke.RepairQueueEntry, rebootEntries []*cke.RebootQueueEntry, nodeStatuses map[string]*cke.NodeStatus, constraints *cke.Constraints) []*cke.RepairQueueEntry {
	...
	for _, machine := range machines {
		if len(machine.Spec.IPv4) == 0 {
			log.Warn("ignore non-healthy machine w/o IPv4 address", map[string]any{
		...
		if recent[address] {
			log.Warn("ignore recently-repaired non-healthy machine", map[string]any{
				"serial":  serial,
				"address": address,
			})
			continue
		...
		if machine.Status.State == StateUnreachable && machine.Status.Duration < float64(constraints.RepairRebootingSeconds) {
			if rebooting[address] {
				log.Info("ignore rebooting unreachable machine", map[string]any{
					"serial":  serial,
					"address": address,
				})
				continue
			...
			if _, ok := nodeStatuses[address]; !ok {
				log.Info("ignore out-of-cluster unreachable machine", map[string]any{
					"serial":  serial,
					"address": address,
				})
				continue
	...
	for i, machine := range newMachines {
		...
		log.Info("initiate sabakan-triggered automatic repair", map[string]any{
			"serial":    machine.Spec.Serial,
			"address":   address,
			"operation": operation,
		})
