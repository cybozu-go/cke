...
// emptyNodeStatus generates an empty NodeStatus.
// This value is equivalent to that generated by op.GetNodeStatus() when no services are running on a node.
func emptyNodeStatus(sshConnected bool) *cke.NodeStatus {
	return &cke.NodeStatus{
		SSHConnected:      sshConnected,
		Etcd:              cke.EtcdStatus{ServiceStatus: cke.ServiceStatus{}, HasData: false, IsAddedMember: false},
		Rivers:            cke.ServiceStatus{},
		EtcdRivers:        cke.ServiceStatus{},
		APIServer:         cke.KubeComponentStatus{ServiceStatus: cke.ServiceStatus{}, IsHealthy: false},
		ControllerManager: cke.KubeComponentStatus{ServiceStatus: cke.ServiceStatus{}, IsHealthy: false},
		Scheduler:         cke.SchedulerStatus{ServiceStatus: cke.ServiceStatus{}, IsHealthy: false},
		Proxy:             cke.ProxyStatus{ServiceStatus: cke.ServiceStatus{}, IsHealthy: false},
		Kubelet:           cke.KubeletStatus{ServiceStatus: cke.ServiceStatus{}, IsHealthy: false},
	}
}
...
func newData() testData {
	...
	status := &cke.ClusterStatus{
		...
		NodeStatuses: map[string]*cke.NodeStatus{
			nodeNames[0]: emptyNodeStatus(true),
			nodeNames[1]: emptyNodeStatus(true),
			nodeNames[2]: emptyNodeStatus(true),
			nodeNames[3]: emptyNodeStatus(true),
			nodeNames[4]: emptyNodeStatus(true),
			nodeNames[5]: emptyNodeStatus(true),
		},
		// Statuses are empty.
...
func (d testData) withSSHNotConnectedCP(indexes ...int) testData {
	...
	for _, i := range indexes {
		// No need to check for out-of-range access. In that case, just panic.
		ip := nodes[i].Address
		d.Status.NodeStatuses[ip] = emptyNodeStatus(false)
	}
	return d
}
